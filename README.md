...
Поэтому поступим просто. Тут первоначально лежит абсолютно пустой проект. Заготовка, такая же как может быть сгенерирована в IDEA. Вот прямо со старой джавой и без каких-либо плюшек, так что POM.xml в любом случае надо будет заменить на свой.

## Что нужно сделать.

1. Сделайте форк этого репозитария.
...
4. В конце вам нужно будет сделать pull resuest из своей ветки (в своем форке) в такую же точно ветку (в этом удаленном репозитарии) и как обычно заполнить форму проектов
